...
import national.soft.impresiones.ILog;
import national.soft.impresiones.R;
import national.soft.impresiones.com.sunmi.ICallback;
import national.soft.impresiones.com.sunmi.IWoyouService;
...
public class AidlUtil {
    ...
    public void connectPrinterService(Context context) {
       ...
       // GeneraLogs.commitToFile("Start service",connService.toString());
       // iLog.escribirlog(connService.toString());
        //GeneraLogs.commitToFile("Intent",intent.toString());
        //iLog.escribirlog(intent.toString());
    }
    ...
    public void printText(String content, float size, boolean isBold, boolean isUnderLine) {
        if (woyouService == null) {
            GeneraLogs.commitToFile("woyou","woyouService is null");
            Toast.makeText(context,R.string.toast_2, Toast.LENGTH_LONG).show();
            return;
        }

        try {
            GeneraLogs.commitToFile("try","entro al try");
            GeneraLogs.commitToFile("try",woyouService.getPrinterModal());
            if (isBold) {
            ...
            GeneraLogs.commitToFile("printTextWithFont","printTextWithFont");

            woyouService.lineWrap(3, null);
            GeneraLogs.commitToFile("lineWrap",woyouService.getPrinterModal());
        } catch (RemoteException e) {
            e.printStackTrace();
            GeneraLogs.commitToFile("RemoteException",e.getMessage() );
        }
